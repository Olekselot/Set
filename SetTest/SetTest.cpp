...
namespace SetTest
{
	...
	public:
		...
		TEST_METHOD(RemoveExistingElement)
		{
			Set<int> set;
			set.add_element(1);
			set.add_element(2);
			...
			Assert::AreEqual((size_t)2, set.size_of_set());
			Assert::IsFalse(set.is_valid(2));
		}
		TEST_METHOD(RemoveNonExistingElement)
		{
			Set<int> set;
			set.add_element(1);
			set.add_element(2);
		...
		TEST_METHOD(TestingSetUnion)
		{
			...
			Assert::AreEqual((size_t)11, D.size_of_set());
